...
input <- "2022/04_camp_cleanup/input.txt"

input |>
  readr::read_lines(skip_empty_rows = TRUE) |>
  tibble::tibble(sections = _) |>
  tidyr::separate(sections, c("s1", "e1", "s2", "e2"), "[,-]") |>
  dplyr::mutate(
    dplyr::across(.fns = as.integer),
    in2 = (s2 <= s1 & e1 <= e2),
    in1 = (s1 <= s2 & e2 <= e1),
    out = in2 | in1
  ) |>
...
aor::day_continue("2022-12-04", "2022/04_camp_cleanup/puzzle.R")

# --- Part Two ---
#
# It seems like there is still quite a bit of duplicate work planned.
# Instead, the Elves would like^[If you like this, you'll \*love\*
# axis-aligned bounding box intersection testing.] to know the number of
