...
input <- "2022/10_cathode_ray_tube/input.txt"

cmds <- readr::read_lines(input, skip_empty_rows = TRUE)
...
for (cmd in cmds) {
  ...
  if (clock %in% c(20, 60, 100, 140, 180, 220)) {
    strength <- strength + clock * register
  }
  ...
  addx <- as.numeric(stringr::str_extract(cmd, "[\\-0-9]+$"))
...
aor::day_continue("2022-12-10", "2022/10_cathode_ray_tube/puzzle.R")
...
# [timing^[While you're at it, go watch everything else by Retro Game
# Mechanics Explained,
# too.]](https://www.youtube.com/watch?v=sJFnWZH5FXc) the CPU
...
for (cmd in cmds) {
  ...
  sprite <- c(register - 1, register, register + 1)
}

for (i in seq(40, 240, by = 40)) {
  cat(paste0(crt[(i - 39):i]))
  cat("\n")
}
