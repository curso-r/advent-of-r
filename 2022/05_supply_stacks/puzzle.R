...
input <- "2022/05_supply_stacks/input.txt"

# Read lines and separate diagram from instructions
lines <- readr::read_lines(input)
diagram <- lines[seq_len(which(lines == "")[1] - 2)]
instructions <- lines[(which(lines == "")[1] + 1):(length(lines))]
...
# Convert diagram into a list, stack by stack
n <- 1
while (tail(diagram, 1) != "") {

  # Iterate over lines of the diagram
  for (i in seq_along(diagram)) {

    # Line is empty
    if (diagram[i] == "") next()

    # Extract crate and add it to the list
    crate <- stringr::str_sub(diagram[i], 2, 2)
    if (crate != " ") out[[n]] <- c(out[[n]], crate)

    # Update diagram
    diagram[i] <- stringr::str_sub(diagram[i], 5)
  }

  n <- n + 1
}

# Transform instructions into moves
moves <- instructions |>
  tibble::tibble(move = _) |>
  dplyr::mutate(
    n = as.numeric(stringr::str_extract(move, "(?<=move )[0-9]+")),
    orig = as.numeric(stringr::str_extract(move, "(?<=from )[0-9]+")),
    dest = as.numeric(stringr::str_extract(move, "(?<=to )[0-9]+"))
  ) |>
  dplyr::rowwise() |>
  dplyr::group_split()
...
# Run instructions
for (move in moves) {
  crane <- rev(out[[move$orig]][seq_len(move$n)])
  out[[move$dest]] <- c(as.character(crane[!is.na(crane)]), out[[move$dest]])
  out[[move$orig]] <- out[[move$orig]][-seq_len(move$n)]
}
...
aor::day_continue("2022-12-05", "2022/05_supply_stacks/puzzle.R")
...
# *CrateMover 9001^[It's way better than the old CrateMover 1006.]*.
#
...
# Run instructions
for (move in moves) {
  crane <- out[[move$orig]][seq_len(move$n)]
  out[[move$dest]] <- c(as.character(crane[!is.na(crane)]), out[[move$dest]])
  out[[move$orig]] <- out[[move$orig]][-seq_len(move$n)]
}
