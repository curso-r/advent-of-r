...
# the jungle^[Where there's jungle, there's hijinxs.] journey.
...
input <- "2022/03_rucksack_reorganization/input.txt"

input |>
  readr::read_lines(skip_empty_rows = TRUE) |>
  purrr::map(\(.) { list(
    fst = stringr::str_sub(., end = stringr::str_length(.) / 2),
    snd = stringr::str_sub(., -stringr::str_length(.) / 2)
  )}) |>
  purrr::transpose() |>
  purrr::map(purrr::flatten_chr) |>
  tibble::as_tibble() |>
  dplyr::mutate(
    dplyr::across(.fns = stringr::str_split, pattern = ""),
    inter = purrr::map2_chr(fst, snd, intersect),
    inter = as.numeric(factor(inter, c(letters, LETTERS)))
  ) |>
  dplyr::pull(inter) |>
  sum()

# Once you're done with part 1, run the following line to fetch part 2:
aor::day_continue("2022-12-03", "2022/03_rucksack_reorganization/puzzle.R")
...
input |>
  readr::read_lines(skip_empty_rows = TRUE) |>
  tibble::tibble(ruck = _) |>
  tibble::rowid_to_column() |>
  dplyr::mutate(rowid = rowid - ((rowid - 1) %% 3)) |>
  dplyr::group_by(rowid) |>
  dplyr::mutate(col = c("a", "b", "c")) |>
  dplyr::ungroup() |>
  tidyr::pivot_wider(names_from = col, values_from = ruck) |>
  dplyr::select(-rowid) |>
  dplyr::mutate(
    dplyr::across(.fns = stringr::str_split, pattern = ""),
    inter = purrr::map2(a, b, intersect),
    inter = purrr::map2_chr(inter, c, intersect),
    inter = as.numeric(factor(inter, c(letters, LETTERS)))
  ) |>
  dplyr::pull(inter) |>
  sum()
