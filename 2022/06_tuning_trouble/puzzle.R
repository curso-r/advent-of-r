...
# sparks^[The magic smoke, on the other hand, seems to be contained...
# FOR NOW!].
#
...
input <- "2022/06_tuning_trouble/input.txt"

signal <- readr::read_lines(input, skip_empty_rows = TRUE)

i <- 1
j <- 4
while (j <= stringr::str_length(signal)) {

  # Find duplicates in current window
  dupes <- signal |>
    stringr::str_sub(i, j) |>
    stringr::str_split_1("(?<=.)") |>
  ...
  # Either break or skip to after last duplicate
  if (!any(dupes)) {
    break()
  } else {
    i <- i + max(which(dupes)) - 1
  }
...
aor::day_continue("2022-12-06", "2022/06_tuning_trouble/puzzle.R")
