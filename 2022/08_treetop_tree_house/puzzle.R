...
# with the height of each tree (your puzzle input^[The Elves have already
# launched a quadcopter (your puzzle input).]). For example:
#
#     30373
#     25512
#     65332
...
input <- "2022/08_treetop_tree_house/input.txt"

trees <- readr::read_lines(input, skip_empty_rows = TRUE)

cols <- stringr::str_length(trees[1])
rows <- length(trees)

mat <- trees |>
  stringr::str_split("(?<=.)") |>
  purrr::map(stringr::str_subset, "[0-9]") |>
  purrr::flatten() |>
  purrr::flatten_chr() |>
  as.integer() |>
  matrix(nrow = rows, ncol = cols, byrow = TRUE)

short <- (rows - 1) * 4 # All edges are visible
for (i in 2:(rows - 1)) {
  for (j in 2:(cols - 1)) {

    if (
      all(mat[i, j] > mat[i, ][1:(j - 1)]) ||
      all(mat[i, j] > mat[i, ][(j + 1):cols]) ||
      all(mat[i, j] > mat[, j][1:(i - 1)]) ||
      all(mat[i, j] > mat[, j][(i + 1):rows])
    ) {
      short <- short + 1
    }
...
aor::day_continue("2022-12-08", "2022/08_treetop_tree_house/puzzle.R")
...
get_view <- function(el, dir) {
  visible <- el > dir

  tall <- which(!visible)[1]
  if (!is.na(tall)) {
    visible[tall] <- TRUE
    if (tall + 1 <= length(dir)) visible[tall + 1] <- FALSE
  }

  sum(purrr::head_while(visible, identity))
}
...
for (i in 2:(rows - 1)) {
  for (j in 2:(cols - 1)) {
      ...
      score <- get_view(mat[i, j], rev(mat[, j][1:(i - 1)])) *
               get_view(mat[i, j], mat[, j][(i + 1):rows]) *
               get_view(mat[i, j], mat[i, ][(j + 1):cols]) *
               get_view(mat[i, j], rev(mat[i, ][1:(j - 1)]))
