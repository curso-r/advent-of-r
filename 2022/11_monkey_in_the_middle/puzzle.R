...
# pack goes flying overhead. Monkeys are playing [Keep
...
#     inspects an item. (An operation like `new = old * 5` means that your
#     worry level after the monkey inspected the item is five times
...
input <- "2022/11_monkey_in_the_middle/input.txt"

lines <- input |>
  readr::read_file() |>
  stringr::str_split_1("\n\n") |>
  stringr::str_subset("M")

parse_monkey <- function(str) {

  make_op <- function(str) {
    f <- stringr::str_extract(str, "(?<=old )[*+]")
    n <- as.numeric(stringr::str_extract(str, "(?<=old [*+] )[0-9]+"))

    if (stringr::str_count(str, "old") > 1) {
      \(old) floor((old ** 2) / 3)
    } else if (f == "*") {
      \(old) floor((old * n) / 3)
    } else if (f == "+") {
  ...
  make_test <- function(str) {
    d <- as.numeric(stringr::str_extract(str, "(?<=divisible by )[0-9]+"))
    throw <- str |>
      stringr::str_extract_all("(?<=to monkey )[0-9, ]+") |>
      purrr::pluck(1) |>
      as.numeric()
...
monkeys <- purrr::map(lines, parse_monkey)

for (round in 1:20) {

  for (i in seq_along(monkeys)) {

    for (j in seq_along(monkeys[[i]]$items)) {

      item <- monkeys[[i]]$items[1]
      monkeys[[i]]$items <- tail(monkeys[[i]]$items, -1)

      new <- monkeys[[i]]$op(item)
      nxt <- monkeys[[i]]$test(new)

      monkeys[[i]]$n_inspections <- monkeys[[i]]$n_inspections + 1

      monkeys[[nxt + 1]]$items <- append(monkeys[[nxt + 1]]$items, new)
    }
  }
}
...
aor::day_continue("2022-12-11", "2022/11_monkey_in_the_middle/puzzle.R")
...
#
# With these new rules, you can still figure out the monkey
# business^[Monkey business monkey business monkey business, monkey
# numbers... is this working?] after 10000 rounds. Using the same example
# above:
#
#     == After round 1 ==
...
parse_monkey <- function(str) {
  ...
  make_op <- function(str) {
    ...
    if (stringr::str_count(str, "old") > 1) {
      \(old) (old ** 2) %% (2*7*3*17*11*19*5*13)
    } else if (f == "*") {
      \(old) (old * n) %% (2*7*3*17*11*19*5*13)
    } else if (f == "+") {
      \(old) (old + n) %% (2*7*3*17*11*19*5*13)
    }
  }
