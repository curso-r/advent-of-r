...
#
# The second column, you reason^[Why do you keep guessing?!], must be
# what you should play in response: `X` for Rock, `Y` for Paper, and `Z`
# for Scissors. Winning every time would be suspicious, so the responses
...
input <- "2022/02_rock_paper_scissors/input.txt"

input |>
  readr::read_lines(skip_empty_rows = TRUE) |>
  tibble::tibble(round = _) |>
  tidyr::separate(round, c("opponent", "me")) |>
  dplyr::mutate(
    dplyr::across(.fns = \(.) as.numeric(factor(.))),
    pts = me + ((me - opponent + 1) %% 3) * 3
  ) |>
  dplyr::pull(pts) |>
  base::sum()

# Once you're done with part 1, run the following line to fetch part 2:
aor::day_continue("2022-12-02", "2022/02_rock_paper_scissors/puzzle.R")
...
input |>
  readr::read_lines(skip_empty_rows = TRUE) |>
  tibble::tibble(round = _) |>
  tidyr::separate(round, c("opponent", "outcome")) |>
  dplyr::mutate(
    dplyr::across(.fns = \(.) as.numeric(factor(.)) - 1),
    pts = 1 + outcome * 3 + (opponent + outcome + 2) %% 3
  ) |>
  dplyr::pull(pts) |>
  base::sum()
