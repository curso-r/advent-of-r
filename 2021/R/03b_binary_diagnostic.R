...
# Calculate rating for a gas recursively
gas <- function(df, co2 = TRUE, bit = 1) {

  # Halting coditions
  if (bit > 12 || nrow(df) == 1) return(df)

  # Return the least common element of a vector
  antimode <- function(x) names(sort(table(x)))[1]

  # Choose proper comparison operator
  if (co2) op <- `==` else op <- `!=`

  # Create a new column with current bit and filter the mode + recurse
  df |>
...
# Apply gas() and gas(co2 = FALSE) to readings and summarise with prod()
"2021/data-raw/03b_binary_diagnostic.txt" |>
