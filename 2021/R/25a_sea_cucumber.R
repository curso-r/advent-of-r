...
# did you say submarine? And the only place with that many sea cucumbers would
# be at the very bottom of the Mariana--" You hang up the phone.
#
...
# east-facing cucumber on the left to move), but both sea cucumbers on the right
# have moved (as the east-facing sea cucumber moved out of the way of the
...
# right edge of the map appear on the left edge, and sea cucumbers that move off
# the bottom edge of the map appear on the top edge. Sea cucumbers always check
...
seafloor <- "2021/data-raw/25a_sea_cucumber.txt" |>
  readr::read_lines() |>
  stringr::str_split("") |>
  purrr::flatten_chr() |>
