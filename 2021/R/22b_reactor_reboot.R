...
#
# After running the above reboot steps, 2758514936282235 cubes are on. (Just for
# fun, 474140 of those are also in the initialization procedure region.)
#
# Starting again with all cubes off, execute all reboot steps. Afterward,
# considering all cubes, how many cubes are on?

# Read all steps as a data frame
steps <- "2021/data-raw/22b_reactor_reboot.txt" |>
  readr::read_lines() |>
  stringr::str_split("[ ,]|(\\.\\.)") |>
  purrr::transpose() |>
...
for (i in 2:nrow(steps)) {
  ...
  # Iterate over cuboids already seen
  for (j in seq_len(nrow(cuboids))) {
    ...
    # Add intersection to cuboids (with flipped state)
