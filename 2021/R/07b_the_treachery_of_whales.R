...
# Read vector of positions
positions <- "2021/data-raw/07a_the_treachery_of_whales.txt" |>
  readr::read_lines() |>
  stringr::str_split(",") |>
  purrr::pluck(1) |>
  as.integer()

# Iterate over all positions to find cheapest
cheapest <- Inf
for (pos in max(positions):min(positions)) {

  # Calculate fuel requirement for position
  fuel <- sum(purrr::map_int(positions, ~sum(0:abs(.x - pos))))

  # Swap if this position is cheaper
