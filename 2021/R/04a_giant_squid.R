...
# Parse numbers drawn
draws <- "2021/data-raw/04a_giant_squid.txt" |>
...
# Calculate score of the winner board
winning_score <- function(df, draws) {
  ...
  # If there is at least one complete row or column...
  if (nrow(win) > 0) {

    # Get winner board, sum unmarked numbers and multiply by draws[1]
    output <- df |>
...
"2021/data-raw/04a_giant_squid.txt" |>
