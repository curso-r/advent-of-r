...
# Get illegality score for a line
score_ilegal <- function(line) {
  stack <- flifo::lifo()

  # Go over line until element popped doesn't match expectations
  symbols <- stringr::str_split(line, "")[[1]]
  for (symbol in symbols) {

    # Either push or pop (and calculate score if necessary)
    if (symbol %in% c("(", "[", "{", "<")) {
...
# Read lines and filter only incomplete
lines <- readr::read_lines("2021/data-raw/10b_syntax_scoring.txt")
...
# Get completeness score for a line
score_complete <- function(line) {
  stack <- flifo::lifo()

  # Remove complete part of line
  symbols <- stringr::str_split(line, "")[[1]]
  for (symbol in symbols) {
  ...
  # Go over leftover stack to calculate score
  score <- 0
